...
import {
  LayoutDashboard, ClipboardList, Flag, User, MessageSquare,
  GitBranch, Users, UserPlus, Activity, BarChart2, CheckSquare,
  LogOut, ChevronLeft, ChevronRight, ShieldCheck, BookOpen
} from 'lucide-react';
...
const Section = ({ label, items, collapsed }: { label: string; items: NavItem[]; collapsed: boolean }) => (
  <div className="mb-1">
    {!collapsed && (
    ...
    <div className="space-y-0.5">
      {items.map((item) => <Item key={item.to} item={item} collapsed={collapsed} />)}
    </div>
  </div>
);
...
export default function Sidebar() {
  ...
  return (
    <aside className={`relative flex flex-col h-screen transition-all duration-300 flex-shrink-0
      bg-gradient-to-b from-[#0d1b2a] to-[#1a2744] ${collapsed ? 'w-16' : 'w-60'}`}>
      <button
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-2 space-y-0.5">
        {role === 'INTERN' && <Section label="Intern" items={internItems} collapsed={collapsed} />}

        {role === 'BUDDY' && (
          <>
            <Section label="Buddy" items={buddyItems} collapsed={collapsed} />
            <Section label="Intern" items={internItems.slice(0, 2)} collapsed={collapsed} />
          </>
        )}

        {role === 'TECH_LEAD' && (
          <>
            <Section label="Tech Lead" items={techLeadItems} collapsed={collapsed} />
            <Section label="Buddy" items={[buddyItems[0], buddyItems[1]]} collapsed={collapsed} />
          </>
        )}

        {role === 'MANAGER' && (
          <>
            <Section label="Manager" items={managerItems} collapsed={collapsed} />
            <Section label="Tech Lead" items={[techLeadItems[0], techLeadItems[2]]} collapsed={collapsed} />
          </>
        )}

        {role === 'HR' && (
          <Section label="HR" items={hrItems} collapsed={collapsed} />
        )}
      </nav>
